// based on litemap's own vec_impl.rs, since we're just re'impl'ing it with a slightly different type signature
...
impl<T> PSRAMVec<T> {
    pub const fn new() -> PSRAMVec<T> {
        PSRAMVec(Vec::new_in(esp_alloc::ExternalMemory))
    }

    pub fn with_capacity(capacity: usize) -> PSRAMVec<T> {
        PSRAMVec(Vec::with_capacity_in(capacity, esp_alloc::ExternalMemory))
    }
}

impl<T: Clone> Clone for PSRAMVec<T> {
    fn clone(&self) -> Self {
        let mut new = Vec::with_capacity_in(self.0.len(), esp_alloc::ExternalMemory);
        new.extend_from_slice(&self.0);
        Self(new)
    }
}

impl<T: Debug> Debug for PSRAMVec<T> {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        f.debug_tuple("PSRAMVec").field(&self.0).finish()
    }
}
...
impl<T> Deref for PSRAMVec<T> {
    type Target = Vec<T, esp_alloc::ExternalMemory>;
...
impl<K, V> StoreConstEmpty<K, V> for PSRAMVec<(K, V)> {
    const EMPTY: PSRAMVec<(K, V)> = PSRAMVec::new();
}

impl<K, V> Store<K, V> for PSRAMVec<(K, V)> {
    #[inline]
...
impl<K, V> StoreSlice<K, V> for PSRAMVec<(K, V)> {
    type Slice = [(K, V)];
...
impl<K, V> StoreMut<K, V> for PSRAMVec<(K, V)> {
    #[inline]
...
impl<K: Ord, V> StoreBulkMut<K, V> for PSRAMVec<(K, V)> {
    #[inline]
...
impl<K: Ord, V> StoreFromIterable<K, V> for PSRAMVec<(K, V)> {
...
impl<'a, K: 'a, V: 'a> StoreIterable<'a, K, V> for PSRAMVec<(K, V)> {
...
impl<'a, K: 'a, V: 'a> StoreIterableMut<'a, K, V> for PSRAMVec<(K, V)> {
...
impl<K, V> StoreIntoIterator<K, V> for PSRAMVec<(K, V)> {
    type KeyValueIntoIter = alloc::vec::IntoIter<(K, V), esp_alloc::ExternalMemory>;
...
impl<T> FromIterator<T> for PSRAMVec<T> {
    fn from_iter<I: IntoIterator<Item = T>>(iter: I) -> Self {
        let mut v = Vec::new_in(esp_alloc::ExternalMemory);
        v.extend(iter);
        PSRAMVec(v)
    }
}

impl<K, V> StoreFromIterator<K, V> for PSRAMVec<(K, V)> {}
